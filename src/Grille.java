...
public class Grille {
    ...
    public void creerGrille(List<String> dic) {
        ...
        // Ajouter des murs et initialiser les cases dans la grille
        for (int i = 0; i < ligne; i++) {
            for (int j = 0; j < colonne; j++) {
                if (rand.nextDouble() < 0.2) { // 20% murs
                    grille[i][j] = '#';
                } else {
        ...
        //prendre quelques mot aleatoirement du dic
    ...
    private boolean PeutEtrePlacer(String mot, int x, int y, int direction) {
        ...
        if (direction == 0) { // horizontale
            ...
            for (int i = 0; i < len; i++) {
                if (grille[x][y + i] != '\0' && grille[x][y + i] != mot.charAt(i)) {
                    return false;
                }
            }
            // Placer le mot
            for (int i = 0; i < len; i++) {
                grille[x][y + i] = mot.charAt(i);
            }
        } else if (direction == 1) { // verticale
            if (x + len > ligne) return false;
            for (int i = 0; i < len; i++) {
                if (grille[x + i][y] != '\0' && grille[x + i][y] != mot.charAt(i)) {
                    return false;
                }
            }
            // Placer le mot
            for (int i = 0; i < len; i++) {
                grille[x + i][y] = mot.charAt(i);
            }
        } else if (direction == 2) { // diagonale
            if (x + len > ligne || y + len > colonne) return false;
            for (int i = 0; i < len; i++) {
                if (grille[x + i][y + i] != '\0' && grille[x + i][y + i] != mot.charAt(i)) {
                    return false;
                }
            }
            // Placer le mot
            for (int i = 0; i < len; i++) {
                grille[x + i][y + i] = mot.charAt(i);
            }
        }
        return true;
    }
